...
describe("feature-libs generator", () => {
  ...
  it("should add custom rules to the generated eslint config for each lib type", async () => {
    ...
    for (const eslintConfigPath of eslintConfigPaths) {
      await expect(readGeneratedFile(eslintConfigPath)).toMatchFileSnapshot(
        getSnapshotPath(eslintConfigPath),
      );
    }
  });
  ...
  describe("feature", () => {
    it("should generate the feature vite config with the tailwind plugin", async () => {
      ...
      await expect(
        readGeneratedFile("libs/my-ressources/feature/vite.config.mts"),
      ).toMatchFileSnapshot(
        getSnapshotPath("libs/my-ressources/feature/vite.config.mts"),
      );
    });
  });
  ...
  describe("ui", () => {
    it("should generate the ui vite config with the icons plugin", async () => {
      ...
      await expect(
        readGeneratedFile("libs/my-ressources/ui/vite.config.mts"),
      ).toMatchFileSnapshot(
        getSnapshotPath("libs/my-ressources/ui/vite.config.mts"),
      );
    });
    ...
    describe("storybook", () => {
      it("should generate storybook files for ui libs", async () => {
        ...
        await expect(
          readGeneratedFile("libs/my-ressources/ui/.storybook/main.ts"),
        ).toMatchFileSnapshot(
          getSnapshotPath("libs/my-ressources/ui/.storybook/main.ts"),
        );
        await expect(
          readGeneratedFile("libs/my-ressources/ui/.storybook/preview.ts"),
        ).toMatchFileSnapshot(
          getSnapshotPath("libs/my-ressources/ui/.storybook/preview.ts"),
        );
        await expect(
          readGeneratedFile("libs/my-ressources/ui/.storybook/styles.css"),
        ).toMatchFileSnapshot(
          getSnapshotPath("libs/my-ressources/ui/.storybook/styles.css"),
        );
        await expect(
          readGeneratedFile("libs/my-ressources/ui/tsconfig.storybook.json"),
        ).toMatchFileSnapshot(
          getSnapshotPath("libs/my-ressources/ui/tsconfig.storybook.json"),
        );
      });
